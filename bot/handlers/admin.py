...
async def admin_stats_callback(callback: types.CallbackQuery):
    ...
    # Growth Calculation (Mental visualization)
...
async def show_drivers_by_status_handler(callback: types.CallbackQuery):
    ...
    for d in drivers[:20]: # Show up to 20 for scrolling
        ...
        text = (
            ...
            f"\U0001f4de Tel: `+{d['phone']}`\n"
            f"\U0001f697 Mashina: **{car_name}**\n"
            f"\U0001f4b0 Balans: `{d.get('balance', 0):,} so'm`"
        )
...
# Note: Consolidated search handler is below.
...
async def admin_manage_list(callback: types.CallbackQuery):
    ...
    text = (
        f"🛡 **ADMINLAR RO'YXATI**\n\n"
        f"Qo'shimcha adminlar ID lari:\n`{current_admins if current_admins else 'Yo\'q'}`\n\n"
...
async def add_new_admin_save(message: types.Message, state: FSMContext):
    try:
        ...
        if str(new_id) not in ids:
            ...
            await message.answer(f"✅ Admin qo'shildi: `{new_id}`\n\nYangi admin botni `/admin` komandasi orqali boshqarishi mumkin.", parse_mode="Markdown")
        else:
...
# Note: admin_payments_menu handles F.data == "admin_payments_menu" later.
# We remove the redirect here to avoid shadowed handlers.
...
async def pay_approve(callback: types.CallbackQuery):
    ...
    if pay:
        amount = pay['amount']
        # Check if amount matches Avto Xabar fee
        avto_fee = await db.get_setting("avto_xabar_fee", "25000")
        
        # If it's an Avto Xabar payment, update ad_messages
        if str(amount) == str(avto_fee):
            from datetime import datetime, timedelta
            expires_at = datetime.now() + timedelta(days=30)
            ad = await db.get_ad_message(uid)
            if not ad:
                await db._execute("INSERT INTO ad_messages (user_id, interval_min, is_active, expires_at) VALUES (?, ?, ?, ?)", 
                ...
                if current_exp:
                    if isinstance(current_exp, str):
                        try:
                            ...
                            try: current_exp = datetime.strptime(current_exp, '%Y-%m-%d %H:%M:%S')
                ...
                await db._execute("UPDATE ad_messages SET expires_at = ? WHERE user_id = ?", (new_exp, uid), commit=True)
            
            try:
                await callback.bot.send_message(uid, "✅ <b>Tabriklaymiz!</b>\n\nAvto Xabar xizmati 30 kunga faollashtirildi.", parse_mode="HTML")
            except: pass
        else:
            # Shift balance if it's a regular refill
...
# ==========================================
# 7. MODERN XABAR YUBORISH (ADVANCED BROADCAST)
# ==========================================
...
async def admin_broadcast_set_target(callback: types.CallbackQuery, state: FSMContext):
    ...
    if target == "custom":
        ...
        await callback.message.edit_text(f"📝 **Xabarni yuboring (Text, Photo, Video, File):**\nTarget: `{target.upper()}`", parse_mode="Markdown")
...
async def admin_broadcast_execute(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await callback.message.edit_text(f"🚀 **REKLAMA YUKLANMOQDA...**\nTarget: `{len(u_list)}` kishi", parse_mode="Markdown")
...
# ==========================================
# 8. MOLIYAVIY BOSHQARUV (PAYMENTS HUB)
# ==========================================
...
async def admin_grabber_menu(callback: types.CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"{'🔴 O\'chirish' if grabber_on == '1' else '🟢 Yoqish'}", callback_data="toggle_grabber")],
...
async def toggle_grabber_handler(callback: types.CallbackQuery):
    ...
    await callback.answer(f"🦅 Grabber: {'Yoqildi' if new == '1' else 'O\'chirildi'}")
...
# ==========================================
# Note: Handlers moved or consolidated below.
...
async def admin_search_user_result(message: types.Message, state: FSMContext):
    ...
    if not user:
        await message.answer("❌ **Foydalanuvchi topilmadi.**\nIltimos, ma'lumotlarni tekshirib qayta kiriting.", reply_markup=admin_back_kb(), parse_mode="Markdown")
        return
    ...
    if role == 'driver':
        text += (
            f"🚗 **Mashina:** `{user.get('car_type', '-')}`\n"
            f"💰 **Balans:** `{user.get('balance', 0):,} so'm`\n"
            f"📂 **Sub end:** `{user.get('subscription_end', '-')}`\n"
            f"⚙️ **Status:** `{user.get('status', '-').upper()}`\n"
        )
...
async def extend_sub_handler(callback: types.CallbackQuery):
    ...
    if current_exp:
        if isinstance(current_exp, str):
            try: current_exp = datetime.strptime(current_exp, '%Y-%m-%d %H:%M:%S.%f')
            except: 
                try: current_exp = datetime.strptime(current_exp, '%Y-%m-%d %H:%M:%S')
                except: current_exp = datetime.now()
        new_exp = max(current_exp, datetime.now()) + timedelta(days=30)
    ...
    await callback.message.answer(f"✅ ID: {tid} obunasi uzaytirildi. Yangi muddat: {new_exp.strftime('%Y-%m-%d')}")
    
    try:
        await callback.bot.send_message(tid, f"🚀 **VIP OBUNA UZAYTIRILDI!**\n\nAdmin tomonidan obunangiz 30 kunga uzaytirildi. Yangi muddat: {new_exp.strftime('%Y-%m-%d')}")
    except: pass

# --- MISSING MANAGEMENT HANDLERS ---
...
@router.callback_query(F.data.startswith("set_status_"), IsAdmin())
async def set_driver_status_handler(callback: types.CallbackQuery):
    # set_status_active_{tid}
    parts = callback.data.split("_")
    new_status = parts[2]
    tid = int(parts[3])
    
    await db.update_driver_status(tid, new_status)
    await callback.answer(f"✅ Status o'zgartirildi: {new_status}")
    
    # Xabarni yangilash (yoki o'chirish)
    await callback.message.edit_reply_markup(reply_markup=admin_driver_manage_kb(tid, new_status))
    
    try:
        msg = "✅ Tabriklaymiz! Hisobingiz aktivlashtirildi." if new_status == 'active' else "❌ Arizangiz rad etildi."
        await callback.bot.send_message(tid, msg)
    except: pass


# ==========================================
# 11. FOYDALANUVCHILAR BOSHQARUVI (USERS HUB)
# ==========================================
...
async def admin_show_all_drivers(callback: types.CallbackQuery):
    ...
    # Agar 30 tadan ko'p bo'lsa CSV yuboramiz
    if len(drivers) > 30:
        report = "Telegram ID, Full Name, Phone, Car, Status, Balance, Joined At\n"
        for d in drivers:
            report += f"{d['telegram_id']}, {d.get('full_name','?')}, {d.get('phone','?')}, {d.get('car_type','?')}, {d.get('status','?')}, {d.get('balance',0)}, {d.get('joined_at','')}\n"
...
async def admin_show_all_passengers(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(text, reply_markup=admin_back_kb(), parse_mode="Markdown", disable_web_page_preview=True)
...
async def admin_show_all_users_combined(callback: types.CallbackQuery):
    ...
    for d in drivers:
        report += f"Driver, {d['telegram_id']}, {d.get('full_name','?')}, {d.get('phone','?')}, {d.get('car_type','?')}, {d.get('balance',0)}\n"
    for p in passengers:
        report += f"Passenger, {p['telegram_id']}, {p.get('full_name','?')}, {p.get('phone','?')}, {p.get('status','?')}, {p.get('joined_at','')}\n"
        
    from aiogram.types import BufferedInputFile
    file = BufferedInputFile(report.encode('utf-8'), filename="jonli_taxi_toplam_baza.csv")
    await callback.message.answer_document(file, caption="📂 **MASTER BAZA (Barcha foydalanuvchilar)**\n\n_Ushbu faylda barcha haydovchi va yo'lovchilar ma'lumotlari jamlangan._", parse_mode="Markdown")
    await callback.answer()

# ==========================================
# 11. BUYURTMALAR BOSHQARUVI
# ==========================================
...
async def admin_orders_menu(callback: types.CallbackQuery):
    ...
    for o in orders:
        ...
        text += f"📍 `{o_id}` | **{p_name}** | {o['type'].upper()} | `{o['status']}`\n"
...
# 11.5 PROMO-KODLAR
# ==========================================
@router.callback_query(F.data == "admin_payments_menu", IsAdmin()) # Reusing for simplicity or can be separate
async def admin_payments_extended_menu(callback: types.CallbackQuery):
    # This could be a sub-menu for Payments + Promocodes
    stats = await db.get_stats()
    text = (
        "💰 **MOLIYA & TO'LOVLAR**\n"
        "━━━━━━━━━━━━━━━━━━━━━\n\n"
...
async def add_promocode_code(message: types.Message, state: FSMContext):
    ...
    await message.answer("💰 **Ushbu kod necha so'm chegirma yoki balans berishini yozing:**\n(Faqat raqam, masalan: 5000)")
...
async def add_promocode_finish(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    # Go back to menu logic here if needed
...
async def admin_settings_menu(callback: types.CallbackQuery):
    ...
    try: mt = f"{int(min_taxi):,}"
    except: mt = min_taxi
    try: sf = f"{int(sub_fee):,}"
    except: sf = sub_fee
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text=f"📢 Sub: {mand_sub.upper()}", callback_data="toggle_mand_sub"),
            InlineKeyboardButton(text=f"🛠 Maint: {maint_mode.upper()}", callback_data="toggle_maint_mode")
        ],
...
# ==========================================
# 13. ALOHIDA XABAR & BALANS (DIRECT)
# ==========================================
@router.callback_query(F.data.startswith("admin_msg_user_"), IsAdmin())
async def admin_msg_user_start(callback: types.CallbackQuery, state: FSMContext):
    uid = int(callback.data.split("_")[3])
    await state.update_data(target_uid=uid)
    await callback.message.answer(f"✍️ Foydalanuvchiga ({uid}) yuborilishi kerak bo'lgan xabarni yozing:")
    await state.set_state(AdminState.waiting_direct_message)

@router.message(AdminState.waiting_direct_message, IsAdmin())
async def process_direct_message(message: types.Message, state: FSMContext, bot: Bot):
    data = await state.get_data()
    uid = data['target_uid']
    try:
        await bot.send_message(uid, f"📢 **ADMINSTRATSIYADAN XABAR:**\n\n{message.text}", parse_mode="Markdown")
        await message.answer("✅ Xabar yuborildi!")
    except:
        await message.answer("❌ Xabar yuborib bo'lmadi (Foydalanuvchi botni bloklagan bo'lishi mumkin).")
    await state.clear()

@router.callback_query(F.data.startswith("admin_refill_"), IsAdmin())
async def admin_refill_balance_start(callback: types.CallbackQuery, state: FSMContext):
    uid = int(callback.data.split("_")[2])
    await state.update_data(target_uid=uid)
    await callback.message.answer(f"💰 Foydalanuvchi ({uid}) balansiga qancha qo'shmoqchisiz?\n(Masalan: 50000)")
    await state.set_state(AdminState.waiting_amount_refill)
...
async def process_refill_amount(message: types.Message, state: FSMContext):
    ...
    # Simple direct update logic
    await db._execute("UPDATE drivers SET balance = balance + ? WHERE telegram_id = ?", (amount, uid), commit=True)
    await message.answer(f"✅ Balans o'zgartirildi: `+{amount:,} so'm`", parse_mode="Markdown")
    await state.clear()
...
async def admin_avto_xabar_menu(callback: types.CallbackQuery):
    ...
    if groups == 'all':
        groups_display = "🔄 Barcha guruhlar (Avtomatik)"
    else:
...
async def handle_avto_payment_action(callback: types.CallbackQuery, bot: Bot):
    ...
    if action == "ok":
        ...
        try:
            await bot.send_message(uid, "✅ <b>Avto Xabar to'lovingiz tasdiqlandi!</b>\n\nXizmat 30 kunga faollashtirildi. E'lon kiritishga o'tishingiz mumkin.", parse_mode="HTML")
        except: pass
    else:
        await db.update_payment_status(pid, 'rejected')
        try:
        ...
        try:
            await bot.send_message(uid, "❌ <b>Sizning Avto Xabar to'lovingiz rad etildi.</b>\n\nIltimos, qayta rasm yuboring yoki admin bilan bog'laning.", parse_mode="HTML")
        except: pass
    await callback.answer()
...
# = = = = = = = = = = = = = = = = = = = = = =
# 15. BAZANI YUKLAB OLISH & ADMIN SETUP
# = = = = = = = = = = = = = = = = = = = = = =
...
async def admin_download_db_handler(callback: types.CallbackQuery):
    ...
    if os.path.exists(DB_PATH):
        ...
        await callback.message.answer_document(file, caption="📁 **Asosiy ma'lumotlar bazasi (SQLite)**", parse_mode="Markdown")
...
async def admin_view_logs_handler(callback: types.CallbackQuery):
    ...
    try:
        with open(log_path, 'r', encoding='utf-8') as f:
            # Efficiently read last 1000 characters or last few lines
            f.seek(0, os.SEEK_END)
            size = f.tell()
            f.seek(max(0, size - 3500))
            text = f.read()
            
        if not text: text = "Tizim jurnallari hozircha bo'sh."
            
        header = (
            "📝 **TIZIM JURNALLARI (REAL-TIME LOGS)**\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"Fayl: `{os.path.basename(log_path)}` | Hajmi: `{size / 1024:.1f} KB`\n"
        )
...
async def admin_health_handler(callback: types.CallbackQuery):
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📅 **Server vaqti:** `{now}`\n"
        f"🗄 **Baza hajmi (DB):** `{db_size / (1024*1024):.2f} MB`\n"
        f"🕒 **Uptime kodi:** STABLE\n"
        f"📍 **OS:** `{os.name.upper()}`\n\n"
    ...
    await callback.message.edit_text(text, reply_markup=kb, parse_mode="Markdown")
@router.callback_query(F.data == "admin_blacklist", IsAdmin())
async def admin_blacklist_handler(callback: types.CallbackQuery):
    bl = await db.get_blacklist()
    if not bl:
        await callback.answer("✅ Qora ro'yxat bo'sh. Hamma foydalanuvchilar ochiq.", show_alert=True)
        return
    
    text = "🚫 **QORA RO'YXAT (BLOCKED USERS):**\n"
    text += "━━━━━━━━━━━━━━━━━━━━━━\n\n"
    
    kb = []
    for b in bl:
        tid = b['user_id']
        reason = b.get('reason', 'Noma\'lum xatolik')
        text += f"• `{tid}` | Sabab: _{reason}_\n"
        kb.append([InlineKeyboardButton(text=f"🔓 Unblock: {tid}", callback_data=f"unblock_user_{tid}")])
    
    kb.append([InlineKeyboardButton(text="⬅️ Ortga", callback_data="admin_users_menu")])
...
async def admin_unblock_user_callback(callback: types.CallbackQuery):
    ...
    await admin_blacklist_handler(callback)
