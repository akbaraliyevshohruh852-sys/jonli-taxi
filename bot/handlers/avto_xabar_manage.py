...
async def check_sub(message: types.Message, user_id: int):
    # KeyError: 'expires_at' was fixed in database.py but let's be safe
...
async def process_ad_payment(message: types.Message, state: FSMContext, bot: Bot):
    ...
    # Store user_id and amount for admin to process
    pid = await db.add_payment(message.from_user.id, int(fee), photo_id)
    
    await message.answer("✅ <b>To'lov qabul qilindi!</b>\n\nAdmin tasdiqlashi bilan xizmat ishga tushadi.", reply_markup=main_control_kb(), parse_mode="HTML")
    await state.clear()
...
async def handle_elon_matni(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    if not await check_sub(callback, callback.from_user.id): return
...
async def process_ad_content(message: types.Message, state: FSMContext):
    ...
    await message.answer("✅ <b>E'lon saqlandi va avto-yuborish yoqildi!</b>", reply_markup=main_control_kb(), parse_mode="HTML")
    await state.clear()
...
async def handle_stat(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        f"🔄 Oxirgi yuborilgan: {ad['last_sent'] if ad['last_sent'] else 'Hali yo\'q'}\n"
        f"⚙️ Holati: {'✅ Faol' if ad['is_active'] else '🛑 To\'xtatilgan'}"
    )
    await callback.message.edit_text(text, reply_markup=main_control_kb(), parse_mode="HTML")
...
async def handle_interval(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"⏱ <b>Xabar yuborish oralig'ini tanlang:</b>", 
                                     reply_markup=interval_kb(interval), parse_mode="HTML")
...
async def set_interval_callback(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(f"⏱ <b>Vaqt oralig'i:</b> {new_interval} daqiqa", 
                                     reply_markup=interval_kb(new_interval), parse_mode="HTML")
...
async def handle_start_ad(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Check if any workers exist globally (for logging)
    from core.config import ADMIN_IDS
    all_workers = []
    ...
    await callback.message.edit_text("🚀 <b>Ishga tushdi!</b>\n\nE'loningiz navbatga qo'shildi va guruhlarga yuboriladi.", reply_markup=main_control_kb(), parse_mode="HTML")
...
async def handle_stop_ad(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🛑 <b>To'xtatildi.</b>", reply_markup=main_control_kb(), parse_mode="HTML")
...
async def handle_sel_acc(callback: types.CallbackQuery):
    ...
    # For now just informational, we can add delete functionality later
    await callback.answer("Profil tanlandi. O'chirish funksiyasi tez orada qo'shiladi.", show_alert=True)

@router.callback_query(F.data == "back_to_main") # Helper for secondary menus
...
async def add_groups_cmd(message: types.Message):
    ...
    if len(args) < 2:
        await message.answer("⚠️ Guruh IDlarini vergul bilan yuboring.\n\nMisol:\n<code>/add_groups -100123,-100456</code>", parse_mode="HTML")
        return
